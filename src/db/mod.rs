...
pub trait Assemble: Sized {
    ...
    async fn assemble_many(
        _: &'_ str,
        _: &mut Connection<Db>,
    ) -> Result<Vec<Self>, ErrorResponder> {
        Err(ErrorResponder {
            message: "not implemented".to_string(),
        })
    }
...
#[async_trait]
pub trait AssembleWithArgs<T>: Sized
where
    T: std::marker::Send,
{
    async fn all_with_args<'a>(_: T, _: &mut Connection<Db>) -> Result<Vec<Self>, ErrorResponder>
    where
        T: 'a,
    {
        Err(ErrorResponder {
...
#[async_trait]
pub trait AssembleWithOutput<T>: Sized
where
    T: std::marker::Send,
{
    ...
    async fn assemble_many_with_output(
        _: &mut Connection<Db>,
    ) -> Result<Vec<Self>, ErrorResponder> {
        Err(ErrorResponder {
            message: "not implemented".to_string(),
...
#[async_trait]
pub trait AssembleWithArgsAndOutput<T, U>: Sized
where
    T: std::marker::Send,
{
    async fn all_with_args_and_output<'a>(_: T, _: &mut Connection<Db>) -> Result<U, ErrorResponder>
    where
        T: 'a,
    {
        Err(ErrorResponder {
